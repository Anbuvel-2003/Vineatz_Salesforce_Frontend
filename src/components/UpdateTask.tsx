...
function UpdateTask() {
  ...
  return (
    <div>
    <div className="p-3">
        <h2 className="text-center text-gray-500 text-3xl capitalize font-serif font-bold">Update Task</h2>
    </div>
    <Formik
                    initialValues={{
                      Task_Name: state.value.Task_Name || "",
                      Task_Description: state.value.Task_Description || "",
                      Task_Assigned_To: state.value.Task_Assigned_To || "",
                      Task_Assigned_By: state.value.Task_Assigned_By || "",
                      Task_Deadline: state?.value?.Task_Deadline ? formatDateString(state.value.Task_Deadline) : "",
                      Task_Status: state.value.Task_Status || ""
                    }}
                    ...
                    onSubmit={async (values) => {
                        ...
                        try {
                          const response = await fetch(
                            `${import.meta.env.VITE_LOCALHOST}/api/updatetask/${params.id}`,
                            {
                              method: "PUT",
                              headers: {

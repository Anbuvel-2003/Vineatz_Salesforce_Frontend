...
const Updateuser: React.FC = () => {
  ...
  firstname:state?.value?.first_Name || "",
  lastname: state?.value?.last_Name || "",
  email: state?.value?.Email || "",
  contact: state?.value?.Mobile_Number || "",
  ...
  const handleSubmit = async (values: FormValues) => {
   if (!params.id) {
    ...
    const createapi = await authApi?.UpdateAdmin(
      {
  ...
  return (
    <div className="bg-[#FDFAFE] w-full min-h-screen px-20 py-10">
      <h2 className="text-[20px] font-semibold mb-6">Update Admin</h2>
      <Formik
